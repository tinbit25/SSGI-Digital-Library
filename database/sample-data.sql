...
-- Backend API Testing:
--   - Test retrieving chunks by resource and page (GET /api/resources/{id}/chunks?page_number=94)
--   - Test resolving Qdrant vector hit IDs back to MySQL content during LLM context building
-- ----------------------------------------------------------------------------
INSERT INTO resource_chunks (resource_id, chunk_index, page_number, chunk_text, vector_id, created_at, updated_at) VALUES
(3, 0, 94, 'Section 1: ETRSS Telemetry Framing. Downlink transmission from ETRSS operates in X-band frequency at 8.2 GHz employing QPSK modulation with packet structures strictly adhering to CCSDS standards.', 'qdr_vec_etrss_chunk_00', DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 5 DAY)),
(3, 1, 95, 'Section 2: Orbital Mechanics & Ground Sampling Distance. The multi-spectral camera payload captures Earth imagery with a nadir Ground Sampling Distance (GSD) of 13.75 meters across four visible and near-infrared spectral bands.', 'qdr_vec_etrss_chunk_01', DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 5 DAY)),
(2, 0, 14, 'Chapter 1: Drought Spectral Indices. Agricultural moisture anomalies in Oromia and East Shewa were quantified by contrasting Level-2A surface reflectance temporal NDVI curves against historical 10-year baselines.', 'qdr_vec_drought_chunk_00', DATE_SUB(NOW(), INTERVAL 8 DAY), DATE_SUB(NOW(), INTERVAL 8 DAY)),
(5, 0, 25, 'Module 1: Automated NDVI Processing in GEE. To initiate interactive geospatial cloud processing, instantiate an ee.ImageCollection for COPERNICUS/S2_SR and apply cloud masking using the QA60 bitmask layer.', 'qdr_vec_gee_chunk_00', DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 2 DAY));
